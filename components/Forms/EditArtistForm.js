...
import { getArtist, updateUser } from '../../api/userData';
...
export default function EditArtistForm({ artistObj }) {
  ...
  useEffect(() => {
    getArtist(user.uid).then(setFormInfo);

    if (artistObj.firebaseKey) setFormInfo(artistObj);
  }, [artistObj, user.uid]);
  ...
  return (
    ...
    >
      <h2 className="mt-5" style={{ paddingBottom: '50px' }}>
        {artistObj.firebaseKey ? 'Update' : 'Create'} User
      </h2>
